      ...
      <main className="max-w-5xl mx-auto px-6 space-y-10">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="space-y-2">
            ...
            <p className="text-sm text-muted-foreground font-bold uppercase tracking-widest max-w-lg">Manage automated SOS nodes and rescue protocols.</p>
          </div>
          
          <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
            <DialogTrigger asChild>
              <Button onClick={handleOpenAdd} size="lg" className="h-14 px-8 rounded-2xl bg-primary text-primary-foreground text-xs font-black uppercase tracking-widest shadow-xl shadow-primary/20">
                <UserPlus className="mr-2 h-4 w-4" /> Establish New Node
              </Button>
            </DialogTrigger>
            <DialogContent className="sm:max-w-[340px] rounded-[2.5rem] border-none shadow-2xl p-5 bg-background">
              <DialogHeader>
                <DialogTitle className="text-xl font-black uppercase tracking-tight text-foreground">{editingId ? 'Edit Node' : 'Configure Node'}</DialogTitle>
                <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest">Rescue coordination link details</p>
              </DialogHeader>
              <div className="space-y-4 py-4">
                <div className="space-y-1.5">
                  <Label className="text-[9px] font-black uppercase tracking-widest text-muted-foreground">Full Identity</Label>
                  <Input placeholder="Enter name" className="h-11 rounded-xl bg-muted border-transparent text-xs font-bold" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} />
                </div>
                <div className="space-y-1.5">
                  <Label className="text-[9px] font-black uppercase tracking-widest text-muted-foreground">Relationship</Label>
                  <Select value={formData.relationship} onValueChange={val => setFormData({...formData, relationship: val})}>
                    <SelectTrigger className="h-11 rounded-xl bg-muted border-transparent text-xs font-bold">
                ...
                <div className="space-y-1.5">
                  <Label className="text-[9px] font-black uppercase tracking-widest text-muted-foreground">Phone Node</Label>
                  <div className="flex gap-2">
                    <Select value={countryCode} onValueChange={setCountryCode}>
                      <SelectTrigger className="w-[80px] h-11 rounded-xl bg-muted border-transparent text-xs font-bold">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                    ...
                    <Input placeholder="Phone" className="h-11 rounded-xl bg-muted border-transparent text-xs font-bold flex-1" value={formData.phoneNumber} onChange={e => setFormData({...formData, phoneNumber: e.target.value})} />
                  </div>
                </div>
              </div>
              <DialogFooter>
                <Button onClick={handleSave} className="w-full h-12 rounded-xl bg-primary text-primary-foreground font-black uppercase tracking-widest text-[10px]">
            ...
            {contacts?.map((contact) => (
              <motion.div key={contact.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className={cn("relative p-8 rounded-[2.5rem] border-2 transition-all", contact.isPrimary ? "bg-slate-900 dark:bg-card border-slate-900 dark:border-primary text-white" : "bg-card border-border")}>
                ...
                <div className="space-y-4">
                  ...
                  {!contact.isPrimary && (
                    <Button variant="outline" className="w-full mt-2 h-10 rounded-xl border-border text-muted-foreground font-black uppercase tracking-widest text-[9px]" onClick={() => setPrimary(contact.id)}>
                      Designate Primary
                    </Button>
                  )}
                </div>
        ...
        <div className="bg-muted/30 p-10 rounded-[3rem] border border-border flex flex-col md:flex-row items-center gap-10">
          <div className="h-16 w-16 rounded-full bg-emerald-100 dark:bg-emerald-900/30 flex items-center justify-center text-emerald-600 dark:text-emerald-400 shrink-0">
            <ShieldCheck className="h-8 w-8" />
          </div>
          <div className="space-y-2">
            <h4 className="text-lg font-black uppercase tracking-tight text-foreground">Safety Protocol 114</h4>
